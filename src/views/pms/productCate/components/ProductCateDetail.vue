  ...
  export default {
    ...
    created() {
      getCategorys().then(res=> {
        this.productCate =this.defaultCate.concat(res.data.list)
      })
      if (this.isEdit) {
        getCategoryDetail(this.$route.query.id).then(response => {
          this.productCateInfo = response.data.list;
        });
      } else {
    ...
    methods: {
      ...
      chooseCateFirst() {
        console.log("parent_first",this.productCateInfo.parent_first)
        this.productCate.forEach(item=> {
          console.log(this.productCateInfo.parent_first,item)
          if(this.productCateInfo.parent_first==item.parent) {
  
            this.selectProductCateList = this.defaultCate.concat(item.subCategory)
          }
        })
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$confirm('是否提交数据', '提示', {
              ...
              let {name,parent,level,isTab} = this.productCateInfo
              console.log("productCateInfo",this.productCateInfo)
              let params = {
                name,parent,level,isTab
              }
              if (this.isEdit) {
