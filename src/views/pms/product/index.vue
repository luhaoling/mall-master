<template> 
  <div class="app-container">
    <el-card class="filter-container" shadow="never">
      ...
      <div style="margin-top: 15px;float:center">
        <el-form :inline="true" :model="search" label-width="140px ">
          <el-form-item label="输入搜索：">
            <el-input style="width: 450px" v-model="search.keyword" placeholder="商品名称"></el-input>
    ...
    <div class="table-container">
      <el-table ref="productTable"
        ...
        <el-table-column label="商品品牌" align="center">
          <template slot-scope="scope">
            <p>{{scope.row.brandsId}}</p>
          </template>
        </el-table-column>
        <el-table-column label="商品分类" align="center">
          <template slot-scope="scope">
            <p>{{scope.row.categoryId}}</p>
          </template>
        </el-table-column>
        <el-table-column label="价格/货号" width="120" align="center">
          <template slot-scope="scope">
            <p>价格：￥{{scope.row.shopPrice}}</p>
...
<script>
  ...
  import {getGoods,deleteGoods,getBrands,getCategorys,putGoodsStatus,getBrandsByCate,searchGood } from '@/apis/goods'
  ...
  export default {
    ...
    methods: {
      ...
      searGoods(){
        searchGood(this.search).then(response=>{
          this.listLoading = false;
          console.log("goods_list")
          console.log(response.data)
          console.log(response.data.list)
      ...
      handlePublishStatusChange(paramname,index, row) {
          if (paramname=='hot') {
              ...
              this.updateNewStatus(paramname,true, row);
          }
      ...
      handleDelete(index, row){
        this.$confirm('是否要进行删除商品?', '提示', {
          ...
          deleteGoods(this.delete).then(res=> {
            this.$message({
            message: '删除成功',
            type: 'success',
            duration: 1000
      ...
      updatePublishStatus(paramname,param, row) {
        ...
        console.log("lgoggdaHotHot",param)
        putGoodsStatus(param).then(response => {
          this.$message({
            message: '修改成功',
            type: 'success',
            duration: 1000
      ...
      updateNewStatus(newStatus, param,row) {
        console.log('数据',row)
     ...
     console.log("lgoggdaNewNew",param)
     putGoodsStatus(param).then(response => {
       this.$message({
         message: '修改成功',
         type: 'success',
